...
window.onload = function() {
        ...
        var oSec3 = document.getElementById('sec3');
        var oS2Opt = oSec2.getElementsByTagName('option');
        var oS3Opt = oSec3.getElementsByTagName('option');
        var arr = [{
            name: "北京市",
            city: [{
                name: "北京市",
        ...
        for(var i = 0; i < arr.length; i++) {
            var sec1Option = document.createElement('option');
            sec1Option.value = i + 1;
            sec1Option.innerHTML = arr[i].name;
            oSec1.appendChild(sec1Option);
        }
        oSec1.onchange = function() {
            for(var i = oS2Opt.length - 1; i > 0; i--) {
                oSec2.removeChild(oS2Opt[i]);
                console.log(oS2Opt.length)
            }
            city(this);
        }
        oSec2.onchange = function() {
            var oSec2Option = oSec2.getElementsByTagName('option');
            for(var x = oS3Opt.length - 1; x > 0; x--) {
                oSec3.removeChild(oS3Opt[x]);
            }
            for(var i = 0; i < arr.length; i++) {
                if(arr[i].name == oSec1.options[oSec1.selectedIndex].text) {
                    var arr2 = arr[i].city;
                    //console.log(arr2);
                    for(var y = 0; y < arr2.length; y++) {
                        if(arr2[y].name == oSec2.options[oSec2.selectedIndex].text) {
                            var arr3 = arr2[y].districtAndCounty;
                            //console.log(arr3);
                            for(var j = 0; j < arr3.length; j++) {
                                var Option = document.createElement('option');
                                Option.value = j + 1;
                                Option.innerHTML = arr3[j];
                                if(oSec2.nextSibling == 1) {
                                    (oSec2.nextSibling).appendChild(Option);
                                } else {
                                    (oSec2.nextSibling.nextSibling).appendChild(Option);
                                }
                            }
        ...
        function city(obj) {
            for(var i = 0; i < arr.length; i++) {
                if(arr[i].name == obj.options[obj.selectedIndex].text) {
                    var arr1 = arr[i].city;
                    for(var j = 0; j < arr1.length; j++) {
                        // console.log(arr1[j].name)
                        var Option = document.createElement('option');
                        Option.value = i + 1;
                        Option.innerHTML = arr1[j].name;
                        // console.log(Option.value);
                        if(obj.nextSibling == 1) {
                            (obj.nextSibling).appendChild(Option);
                        } else {
                            (obj.nextSibling.nextSibling).appendChild(Option);
                        }
                    }
                }
            }
